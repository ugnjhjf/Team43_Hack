import { generateRecommendations } from "./scripts/chatgpt.mjs";
...
window.onload = function () {
      ...
      set.forEach((itemName) => {
        ...
        if (filteredItem) {
          let cardHTML = `<p style="font-family: 'Roboto', sans-serif;" class="fs-4 mx-3 fw-semibold">
                    ...
                    ${filteredItem.item} ${filteredItem.detail}. ${
            filteredItem.tips
          }
                    </p>`;
  ...
  async function main() {
    ...
    destination.addEventListener("change", async function () {  // 注意这里的 async
      let str;
  
      if (this.value !== "default") {
        str = await generateRecommendations(this.value);  // 使用 await 调用 generateRecommendations
      } else {
        str = "Beijing, the heart of China... [GPS Bicycle meter,Sun protection umbrella,Walking Stick,Shaped Eye Mask]";
      }
    ...
    if (main() == null){
      let str ="Beijing, the heart of China... [GPS Bicycle meter,Sun protection umbrella,Walking Stick,Shaped Eye Mask]";
    }else{
      str = main();
    }
    extractItemsFromString(str);
  ...
  function extractItemsFromString(str) {//提取方括号
    let regex = /\[(.*?)\]/;
  ...
  $(document).ready(function () {
    $.get("assets/goods.json", function (data) {
      ...
      goodsList.forEach((drink) => {
        let goodfind = data.find((item) => item.name=== drink);
        $("#goods-menu").append(
          `<div class='col d-flex justify-content-center' id="${id}"></div>`
        );
      ...
      $("#goods-menu")
        .html("<div></div>")
        .removeClass("row-cols-1 row-cols-sm-2 row-cols-md-4");
      $("#goods-menu > div")
